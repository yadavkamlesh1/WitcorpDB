            ...
            data.forEach(row => {
                ...
                tbody.innerHTML += `
                    <tr class="group transition-all hover:bg-slate-50/80">
                        ...
                        <td class="p-4 relative group"><div class="text-slate-500 text-[11px] italic max-w-[180px] truncate font-medium cursor-pointer">${row.remarks || '---'}</div><div class="absolute left-0 top-full mt-2 hidden group-hover:block z-50 bg-slate-900 text-white text-xs p-4 rounded-2xl shadow-2xl w-[320px] break-words leading-5 border border-slate-700">${row.remarks || 'No remarks available'}</div></td>
