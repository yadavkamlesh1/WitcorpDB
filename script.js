            ...
            data.forEach(row => {
                ...
                tbody.innerHTML += `
                    <tr class="group transition-all hover:bg-slate-50/80">
                        ...
                        <td class="p-4 relative group"><div class="text-slate-500 text-[11px] italic max-w-[180px] truncate font-medium cursor-pointer">${row.remarks || '---'}</div><div class="absolute left-1/2 -translate-x-1/2 bottom-full mb-3 hidden group-hover:block z-50 bg-white/95 backdrop-blur-xl text-slate-700 text-xs p-4 rounded-2xl shadow-2xl w-[320px] break-words leading-5 border border-slate-200">${row.remarks || 'No remarks available'}</div></td>
